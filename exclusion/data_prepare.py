...
def extract_real_cubic_from_mhd(dcim_path,annatation_file,plot_output_path,normalization_output_path):
    ...
    for img_file in file_list:
        mini_df = df_node[df_node["file"]==img_file] #get all nodules associate with file 获得所有结节文件
        ...
        if mini_df.shape[0]>0: # some files may not have a nodule--skipping those
            ...
            for node_idx, cur_row in mini_df.iterrows():
                ...
                try:
                    ...
                    # these following are the standard data as input of CNN 以下是作为CNN输入的标准数据
                    truncate_hu(imgs1)
                    truncate_hu(imgs2)
                    truncate_hu(imgs3)
    ...
    for img_file in file_list:
        ...
        if mini_df.shape[0]>0 and num<10000: # some files may not have a nodule--skipping those
            ...
            for node_idx, cur_row in mini_df.iterrows():
                ...
                node_z = cur_row["coordZ"]
                nodule_pos_str = str(node_x) + "_" + str(node_y) + "_" + str(node_z)
                center = np.array([node_x, node_y, node_z])  # nodule center 结节中心
                v_center = np.rint((center - origin) / spacing)  # nodule center in voxel space ( x,y,z ordering)
                # false nodule 假结节
                # every nodules saved into size of 20x20x6,30x30x10,40x40x26  每个结节存储成三种尺寸
...
def check_nan(path):
    '''
     a function to check if there is nan value in current npy file path 一个函数来检查当前npy文件路径中是否有nan值 ？
    :param path:
    :return:
    '''
...
def plot_cubic(npy_file):
    '''
       plot the cubic slice by slice 逐片绘制立方体
...
# LUNA2016 data prepare ,first step: truncate HU to -1000 to 400 截断HU到-1000到400
def truncate_hu(image_array):
    image_array[image_array > 400] = 0
    image_array[image_array <-1000] = 0

# LUNA2016 data prepare ,second step: normalzation the HU 正则化
...
def get_test_batch(path):
    '''
            prepare every batch file data and label 【test】 测试批文件准备
    :param path:
    :return:
    '''
    #files = [os.path.join(path,f) for f in os.listdir(path)]
    files = get_all_filename(path,20)
    ...
    for npy in files:
        try:
            if len(batch_label)<32:
                ...
                #print 'get_test_batch,arr.shape = ',arr.shape
...
def get_train_batch(batch_filename):
    '''
      prepare every batch file data and label 【train】 训练批文件准备
    ...
    for npy in batch_filename:
        try:
            arr = np.load(npy)
            arr = arr.transpose(2,1,0)   
            #print 'get_train_batch,arr.shape = ',arr.shape
            batch_array.append(arr)
...
def angle_transpose(file,degree,flag_string):   #角度旋转
    '''
