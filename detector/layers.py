...
class Loss(nn.Module):
    def __init__(self, num_hard = 0):
        ...
        self.classify_loss = nn.BCELoss() # 分类损失函数使用了二分类交叉熵损失函数
        self.regress_loss = nn.SmoothL1Loss() # 回归损失函数使用了smoothL1
    ...
    def forward(self, output, labels, train = True):
        ...
        neg_output = output[:, 0][neg_idcs] # label是反例的输出中只有class部分才参与计算,其余地方需要回归的值不参与计算
        neg_labels = labels[:, 0][neg_idcs] 
        
        if self.num_hard > 0 and train: # 这里使用了hard negative mining
